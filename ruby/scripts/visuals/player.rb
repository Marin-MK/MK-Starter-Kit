class Visuals
  ...
  class Player < BaseCharacter
    ...
    def initialize(game_player)
      ...
      @sprite.x = System.width / 2
      @sprite.y = System.height / 2 + 16
      @fake_anim = nil
    ...
    def move(xdiff, ydiff, xtilediff, ytilediff)
      ...
      if xtilediff != 0
        xdiff *= 32
        $visuals.maps.each_value { |m| m.x -= xtilediff * 32 }
        $visuals.map_renderer.move_x(xdiff)
      end
      if ytilediff != 0
        ydiff *= 32
        $visuals.maps.each_value { |m| m.y -= ytilediff * 32 }
    ...
    def update_fake_movement
      ...
      if @fake_anim && !@game_character.idle_animation
        ...
        if @fake_anim == 0
          ...
          if @stop_fake_anim
            @fake_anim = nil
            @stop_fake_anim = false
            next_frame if (@sprite.src_rect.x.to_f / @sprite.bitmap.width * 4) % 2 != 0
          else
            @fake_anim = 16
